...
public class Worlds
{
	private static List<World> lookup()
	{
		...
		WorldResult lookup = Main.INSTANCE.getWorldService().getWorlds();
		if (lookup == null)
		{
		...
		{
			World world = Main.INSTANCE.getClient().createWorld();
	...
	public static void hopTo(World world, boolean spam) throws Exception {
		...
		Main.INSTANCE.getClient().interact(1, MenuAction.CC_OP.getId(), world.getId(), WidgetInfo.WORLD_SWITCHER_LIST.getId());
		if (!spam)
		{
			Time.sleepUntil(() -> Game.getState() == GameState.HOPPING, 3000);
		}
	...
	public static void openHopper()
	{
		...
		Main.INSTANCE.getClient().interact(1, MenuAction.CC_OP.getId(), -1, WidgetInfo.WORLD_SWITCHER_BUTTON.getId());
	}

	public static void openLobbyWorlds()
	{
		Main.INSTANCE.getClient().loadWorlds();
		Main.INSTANCE.getClient().setWorldSelectOpen(true);
	}

	public static void closeLobbyWorlds()
	{
		Main.INSTANCE.getClient().setWorldSelectOpen(false);
	}
