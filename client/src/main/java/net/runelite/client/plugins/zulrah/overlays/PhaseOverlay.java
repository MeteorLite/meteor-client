...
public class PhaseOverlay
   ...
   @Inject
   public PhaseOverlay(ZulrahPlugin plugin, ZulrahConfig config) {
      this.plugin = plugin;
      this.config = config;
      ...
      this.getPanelComponent().setPreferredSize(new Dimension(56, 56));
      this.getPanelComponent().setBorder(new Rectangle());
      this.getPanelComponent().setGap(new Point(1, 1));
      this.getPanelComponent().setOrientation(ComponentOrientation.HORIZONTAL);
   }
   ...
   public Dimension render(Graphics2D graphics) {
      ...
      components.forEach(i -> getPanelComponent().getChildren().add(i));
   ...
   private void displayRotationName(Graphics2D graphics) {
      Rectangle bounds = getPanelComponent().getBounds();
