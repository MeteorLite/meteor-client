 ...
 */(private val npcId: Int) {
    ...
    companion object {
        private val byId: MutableMap<Int, NpcID?> = HashMap()
        private val byName: MutableMap<String, NpcID?> = HashMap()

        init {
            for (npc in entries) {
                require(byId.put(npc.id(), npc) == null) { "duplicate id: " + npc.id() }
                require(byName.put(sanitizeName(npc.name), npc) == null) { "duplicate sanitized name: " + npc.id() }
            }
        }

        fun getById(id: Int): NpcID? {
            return byId.getOrDefault(id, NOBODY)
        }

        fun getByName(name: String): NpcID? {
