 ...
 */(private val itemId: Int) {
    ...
    companion object {
        const val maxAuthentic: Int = 1289
        const val maxCustom: Int = 1501

        private val byId: MutableMap<Int, ItemID?> = HashMap()
        private val byName: MutableMap<String, ItemID?> = HashMap()

        init {
            for (item in entries) {
                require(byId.put(item.id(), item) == null) { "duplicate id: " + item.id() }
                require(byName.put(sanitizeName(item.name), item) == null) { "duplicate sanitized name: " + item.id() }
            }
        }

        fun getById(id: Int): ItemID? {
            return byId.getOrDefault(id, NOTHING)
        }

        fun getByName(name: String): ItemID? {
