...
import static orsc.graphics.two.MudClientGraphics.mudClient;
...
public final class mudclient implements Runnable {
	...
	private int[] realSkillLevels;
	private int[] skillXPs;
	private boolean experienceOff = false;
	private GameState gameState1 = GameState.LOGIN;
	...
	private int[] levelXPs = new int[S_PLAYER_LEVEL_LIMIT];
	...
	private int[] boostedSkillLevels;
		...
		try {
			try {
				...
				while (this.threadState >= 0) {
					...
					var6 &= 255;
					if (reposition()) {
						if (this.gameState1 == GameState.LOGIN) {
	...
	private void createLoginPanels(int var1) {
		try {
			...
			if (S_WANT_HIDE_IP) {
				this.settingsHideIP = loadHideIp();
			...
			if (!this.errorLoadingData) {
				if (this.errorLoadingMemory) {
					...
					try {
						...
						if (this.gameState1 == GameState.GAME) {
	...
	private void chooseOption(int optionIdx) {
		...
		this.packetHandler.getClientStream().bufferBits.putByte(optionIdx);
		this.packetHandler.getClientStream().finishPacket();
	}
			...
			if (var1 == 13) {
				if (this.deathScreenTimeout != 0) {
					...
					if (S_SIDE_MENU_TOGGLE && C_SIDE_MENU_OVERLAY) {
						...
						//i += 14;
						this.getSurface().drawString(
							"Hits: " + this.boostedSkillLevels[3] + "@gre@/@whi@" + this.realSkillLevels[3], 7, i, 0xffffff, 1);
						i += 14;
						this.getSurface().drawString(
							"Prayer: " + this.boostedSkillLevels[5] + "@gre@/@whi@" + this.realSkillLevels[5], 7, i, 0xffffff, 1);
						i += 14;
						if (C_TOTAL_NPC_KC) {
							this.getSurface().drawString(
			...
			if (this.inputTextFinal.length() <= 0 && !this.inputX_OK) {
				...
				if (this.inputX_Action == InputXAction.TRADE_OFFER) {
					...
					try {
						...
						if (var4 > this.levelXPs[Config.S_PLAYER_LEVEL_LIMIT - 2]) {
							var4 = this.levelXPs[Config.S_PLAYER_LEVEL_LIMIT - 2]; //104273167 is the maximum XP possible in OpenPK, level 120 exp is originalCurveExperienceArray[lvl - 2] / 4.
						}
						this.packetHandler.getClientStream().newPacket(199);
						this.packetHandler.getClientStream().bufferBits.putByte(13);
					...
					try {
						...
						int n280;
						n280 = this.realSkillLevels[pointsSkillId] + var4;
						...
						int nextLev3;
						nextLev20 = var4 - 2;
						nextLev3 = this.levelXPs[this.realSkillLevels[pointsSkillId] + nextLev20];
						var4 = (nextLev3 - this.skillXPs[pointsSkillId]);
					...
					try {
						...
						int n2800;
						n2800 = this.realSkillLevels[pointsSkillId] - var4;
						if (n2800 < 1) {
						...
						int nL0;
						int nL1;
						nL0 = this.realSkillLevels[pointsSkillId] - 2;
						nL1 = this.realSkillLevels[pointsSkillId] - 1;
						if (n2800 == 1) {
							var4 = this.skillXPs[pointsSkillId];
						} else {
							var4 = this.skillXPs[pointsSkillId] - this.levelXPs[nL0 - var4];
						}
						if (var4 < 1) {
							return;
						}
	...
	private void drawExperienceCounter(int skill) {
		...
		if (C_EXPERIENCE_COUNTER_MODE == 1 || skill < 0) {
			for (int i = 0; i < skillCount; i++) {
				totalXp += Integer.toUnsignedLong(this.skillXPs[i]);
			}
			...
			int stringWid = getSurface().stringWidth(3, skillNames[skill] + ": " + realSkillLevels[skill] + ": " + skillXPs[skill]);
			...
			double progress = 0;
			if (realSkillLevels[skill] != S_PLAYER_LEVEL_LIMIT) {
				tilLvl = this.levelXPs[realSkillLevels[skill] - 1] - this.skillXPs[skill];
				baseTilLvl = this.levelXPs[realSkillLevels[skill]] - this.levelXPs[realSkillLevels[skill] - 1];
			...
			if (textColor == 0xFFFFFF) {
				getSurface().drawShadowText(skillNames[skill] + ": " + realSkillLevels[skill] + ": " + skillXPs[skill], (getGameWidth() / 2) - (stringWid / 2) - 4, 15, textColor, 2, false);
			} else {
				getSurface().drawString(skillNames[skill] + ": " + realSkillLevels[skill] + ": " + skillXPs[skill], (getGameWidth() / 2) - (stringWid / 2) - 4, 15, textColor, 2);
			}
			...
			if (C_EXPERIENCE_CONFIG_SUBMENU && mouseX >= x && mouseX <= x + width && mouseY >= 0 && mouseY <= 20) {
				...
				if (textColor == 0xFFFFFF) {
					if (realSkillLevels[skill] == S_PLAYER_LEVEL_LIMIT) {
						...
						this.getSurface().drawShadowText("Next lvl: " + this.levelXPs[realSkillLevels[skill] - 1], x + 3, 33, textColor, 2, false);
					...
					if (realSkillLevels[skill] == S_PLAYER_LEVEL_LIMIT) {
						...
						this.getSurface().drawString("Next lvl: " + this.levelXPs[realSkillLevels[skill] - 1], x + 3, 33, textColor, 2);
						...
						if (id != 1) {
							...
							if (id == 2) {
								...
								if (EntityHandler.getNpcDef(var13).isAttackable()) {
									...
									int playerLevel = (this.realSkillLevels[3] + this.realSkillLevels[2]
										+ this.realSkillLevels[1] + this.realSkillLevels[0] + 27) / 4;
									var11 = "@yel@";
			...
			if (var2 == -74) {
				...
				if (this.m_Ji == 0) {
					...
					for (var10 = 0; var10 < EntityHandler.spellCount(); ++var10) {
						...
						var12 = this.boostedSkillLevels[6];
				...
				if (this.m_Ji == 1) {
					...
					for (var10 = 0; var10 < EntityHandler.prayerCount(); ++var10) {
						var11 = "@whi@";
						if (EntityHandler.getPrayerDef(var10).getReqLevel() > this.realSkillLevels[5]) {
							var11 = "@bla@";
						}
				...
				if (var1) {
					...
					if (var3 >= 0 && var17 >= 0 && var3 < 196 && var17 < 182) {
						...
						if (this.mouseButtonClick == 1 && this.m_Ji == 0) {
							...
							if (var9 != -1) {
								var10 = this.boostedSkillLevels[6];
							...
							if (var9 != -1) {
								...
								if (var10 < EntityHandler.getPrayerDef(var9).getReqLevel()) {
									this.showMessage(false, null,
										...
										0, null);
								} else if (this.boostedSkillLevels[5] == 0) {
									this.showMessage(false, null,
		...
		try {
			...
			if (this.uiTabPlayerInfoSubTab == 0) {
				...
				for (currSkill = 0; currSkill < skillCount; currSkill++) {
					...
					this.getSurface().drawString(this.getSkillNames()[currSkill] + ":@yel@" + this.boostedSkillLevels[i]
						+ "/" + this.realSkillLevels[i], xOffset, yOffset, currentlyHoveredSkill == i ? textColourHovered : textColour, 1);
				...
				if (currentlyHoveredSkill == -1) {
					...
					for (currSkill = 0; currSkill < skillCount; ++currSkill) {
						totalXp += this.skillXPs[currSkill];
						currSkillTotal += this.realSkillLevels[currSkill];
					}
					...
					heightMargin += 12;
					int nextLevelExp = this.levelXPs[0];

					for (int currLevel = 0; currLevel < S_PLAYER_LEVEL_LIMIT - 1; ++currLevel) {
						if (this.levelXPs[currLevel] <= this.skillXPs[currentlyHoveredSkill]) {
							nextLevelExp = this.levelXPs[currLevel + 1];
						}
					}

					this.getSurface().drawString("Total xp: " + this.skillXPs[currentlyHoveredSkill], 5 + x, heightMargin, textColour,
						1);
					heightMargin += 12;
					this.getSurface().drawString("Next level at: " + nextLevelExp, 5 + x, heightMargin, textColour, 1);
					if (S_WANT_EXP_INFO) {
						heightMargin += 12;
						this.getSurface().drawString("Xp to next level: " + (nextLevelExp - this.skillXPs[currentlyHoveredSkill]), 5 + x, heightMargin, textColour, 1);
					}
				}
			}
				...
				if (loginScreenNumber == 1) {
					...
					if (isAndroid() || Remember()) {
						if (this.panelLogin.isClicked(this.rememberButtonIdx)) {
							...
							boolean temp = saveCredentials(this.panelLogin.getControlText(this.controlLoginUser) + "," + this.panelLogin.getControlText(this.controlLoginPass));
...
//							}
	...
	private void jumpToLogin() {
		try {
			this.logoutTimeout = 0;
			this.loginScreenNumber = 0;
			this.gameState1 = GameState.LOGIN;
	...
	private void resetGame(int var1) {
		try {
			...
			this.gameState1 = GameState.GAME;
		...
		try {
			...
			if (!clientPort.drawLoading(2)) {
				...
				if ((Config.SERVER_IP != null)) {
					...
					String ip = loadIP(); // loads based on Cache/ip.txt
					int port = loadPort(); // loads based on Cache/port.txt
					System.out.println(" ");
					System.out.println(" ");
					System.out.println("Fetching server configs from " + ip + ":" + port);
					System.out.println(" ");
	...
	void setExperienceArray() {
		...
		for (int i = 0; i < S_PLAYER_LEVEL_LIMIT; ++i) {
			...
			this.levelXPs[i] = (int)((experience & 0xffffffffL) / 4);
		}
	}

	public int[] getLevelXPs() {
		return this.levelXPs;
	}
	...
	final void continueStartGame(byte var1) {
		...
		try {
			...
			if (!this.errorLoadingData) {
				...
				this.realSkillLevels = new int[skillCount];
				this.boostedSkillLevels = new int[skillCount];
				this.skillXPs = new int[skillCount];
	...
	public int getPlayerStatCurrent(int skill) {
		return boostedSkillLevels[skill];
	}
