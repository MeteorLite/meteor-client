...
public class PacketHandler {
	...
	private void setServerConfiguration() {
		...
		if (!mc.gotInitialConfigs || Config.isAndroid()) {
			props.setProperty("SERVER_IP", mudclient.loadIP()); // 0
			props.setProperty("SERVER_PORT", String.valueOf(mudclient.loadPort())); // 0
		}
		...
		if (!mc.gotInitialConfigs) {
			mc.setLevelXPs(new int[Config.S_PLAYER_LEVEL_LIMIT]);
