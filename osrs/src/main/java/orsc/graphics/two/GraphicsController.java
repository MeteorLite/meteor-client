			...
			if (width > 0 && height > 0) {
				if (antiAliased) {
					this.plotLetterAntialiased(fontData, color, width, startPixel, height, srcStride, rowStride,
						this.pixels, dataAddr);
				} else {
					this.plotLetter(color, this.pixels, startPixel, rowStride, height, width, dataAddr, fontData,
						srcStride);
				}
			}
		...
		try {
			...
			try {
				...
				this.plot_tran_scale(var19, var11, width, (byte) -61, scaleY, spriteWidth, scaleX, height, var14,
					sprite.getPixels(), 0, var10, var15, var7, this.pixels);
	...
	public final void a(Sprite sprite, int var2, int var3, int var4, int var5) {
		try {
			...
			if (var9 > 0 && var8 > 0) {
				...
				// } else {
				this.a(var7, var8, var6, sprite.getPixels(), 0, var4, var14, this.pixels, -107, var11,
					var10, var9);
				// }
	...
	public final void spriteClip3(int var1, int var2, Sprite sprite, int var4, int var5, byte var6, int var7) {
		try {
			...
			try {
				...
				this.a(var11, var12, var7, var10, var15, sprite.getPixels(), var14, this.pixels, 0,
		...
		try {
			for (int var8 = var4; var8 < var4 + var6; ++var8) {
				for (int var9 = var3; var3 + var1 > var9; ++var9) {
						...
						if (var14 >= 0 && this.width2 > var14) {
							for (int var15 = var9 - var2; var15 <= var9 + var2; ++var15) {
								if (var15 >= 0 && this.height2 > var15) {
									int inLetter = this.pixels[this.width2 * var15 + var14];
									var12 += 0xFF & inLetter;
									++var13;
					...
					this.pixels[var8 + var9 * this.width2] = var12 / var13 + (var10 / var13 << 16)
						+ (var11 / var13 << 8);
				}
			}
		...
		try {
			...
			for (int x = xOffset; x < xOffset + width; x++) {
				for (int y = yOffset; y < yOffset + height; y++) {
					pixels[pixel++] = this.pixels[x + y * width2];
				}
			}
	...
	public final void drawBoxAlpha(int x, int y, int width, int height, int color, int alpha) {
		try {
			...
			for (int yi = 0; yi < height; yi += var16) {
				for (int xi = -width; xi < 0; ++xi) {
					int o1 = mixOld * (this.pixels[pxi] & 255);
					int o3 = mixOld * ((0xFF0000 & this.pixels[pxi]) >> 16);
					int o2 = mixOld * ((0xFF00 & this.pixels[pxi]) >> 8);
					int var20 = (o1 + n1 >> 8) + (o2 + n2 >> 8 << 8) + (n3 + o3 >> 8 << 16);
					this.pixels[pxi++] = var20;
				}
		...
		try {
			...
			for (int py = startY; endY >= py; py += yStep) {
				...
				for (int px = startX; endX >= px; ++px) {
					int destG = (this.pixels[rowStart] & 255) * destAlpha;
					int destB = destAlpha * ((0xFF00 & this.pixels[rowStart]) >> 8);
					int destR = destAlpha * ((this.pixels[rowStart] & 0xFF0000) >> 16);
					int finalColor = (destG + srcG >> 8) + (srcB + destB >> 8 << 8) + (srcR + destR >> 8 << 16);
					this.pixels[rowStart++] = finalColor;
				}
			}
		...
		if (this.clipLeft <= x && x < this.clipRight) {
			...
			if (height > 0) {
				...
				for (int i = 0; height > i; ++i) {
					this.pixels[pxOffset + this.width2 * i] = color;
				}
			}
		}
			...
			for (int var41 = var27; var41 < var28; ++var41) {
				...
				if (var43 - var42 > 0) {
					...
					if (!this.interlace || (var41 & 1) == 0) {
						if (sprite.requiresShift()) {
							this.plot_trans_horiz_line(var47, var42 - var43, var44, var40, this.pixels, var46,
								var42 + var39, var37, var45);
						} else {
							this.plot_horiz_line(var42 - var43, var37, this.pixels, var45, var46, var44, var40,
								var47, var42 + var39);
						}
					}
	...
	public final void drawSprite(Sprite sprite, int x, int y) {
		try {
			...
			if (var8 > 0 && var7 > 0) {
				...
				this.a(var8, this.pixels, var13, var7, 0, var6, (byte) 123, var5, sprite.getPixels(), var9,
					var10);
		...
		try {
			try {
				...
				this.plot_scale_black_mask(sprite.getPixels(), heightStep, scaleX, 0, srcStartY,
					this.pixels, (byte) 78, scaleY, destHeight, srcStartX, destRowStride, destWidth, spriteWidth,
					destHead);
	...
	public final void fade2black(int var1) {
		try {
			...
			for (int i = 0; var4 > i; ++i) {
				int var2 = 0xFFFFFF & this.pixels[i];
				this.pixels[i] = FastMath.bitwiseAnd(var2 >>> 4, 0x0F0F0F)
	...
	public final void setPixel(int x, int y, int val) {
		try {
			...
			if (this.clipLeft <= x && this.clipTop <= y && this.clipRight > x && this.clipBottom > y) {
				this.pixels[x + this.width2 * y] = val;
			}
		} catch (RuntimeException var6) {
					...
					if (null != e.getPixels()) {
						if (mirrorX) {
							this.plot_tran_scale_with_mask(dummy ^ 74, e.getPixels(), scaleY, 0,
								...
								this.pixels, height, destColumnSkewPerRow, destRowHead, -scaleX, destFirstColumn,
