		...
		if (TextureProvider.localPlayer != var0) {
			if (Client.menuOptionsCount < 400) {
				...
				if (var0.skillLevel == 0) {
					var4 = var0.actions[0] + var0.username + var0.actions[1] + WidgetDefinition.method1745(var0.combatLevel, TextureProvider.localPlayer.combatLevel) + " " + " (" + "level-" + var0.combatLevel + ")" + var0.actions[2];
				} else {
				...
				if (Client.isItemSelected == 1) {
					class153.insertMenuItemNoShift("Use", Client.field598 + " " + "->" + " " + Client.colorStartTag(16777215) + var4, 14, var1, var2, var3);
				} else if (Client.isSpellSelected) {
					if ((class31.field82 & 8) == 8) {
						class153.insertMenuItemNoShift(Client.field599, Client.field600 + " " + "->" + " " + Client.colorStartTag(16777215) + var4, 15, var1, var2, var3);
					}
				} else {
					for (var5 = 7; var5 >= 0; --var5) {
						if (Client.playerMenuActions[var5] != null) {
							...
							if (Client.playerMenuActions[var5].equalsIgnoreCase("Attack")) {
								...
								} else if (Client.playerAttackOption == AttackOption.field1102 && var0.isClanMember()) {
									var6 = 2000;
								}
