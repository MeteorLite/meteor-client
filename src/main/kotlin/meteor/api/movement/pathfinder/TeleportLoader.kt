...
object TeleportLoader {
    ...
    fun buildTeleports(): List<Teleport> {
        ...
        fun getWildyLevel(): Int {
            val wildyLevelWidget = Widgets[WidgetInfo.PVP_WILDERNESS_LEVEL]
        ...
        val teleports: MutableList<Teleport> = ArrayList()
            ...
            if (getWildyLevel() <= 20) {
                if (ringOfDueling()) {
                    teleports.add(
                        Teleport(WorldPoint(3315, 3235, 0), 6
                ...
                if (necklaceOfPassage()) {
                    ...
                    teleports.add(
                        Teleport(
                            WorldPoint(3405, 3157, 0), 2
                        ) { jewelryTeleport("Eagles' Eyrie", *NECKLACE_OF_PASSAGE) }
                    )
                }
                for (tab in TeleportTab.values()) {
                    if (tab.canUse() && tab.getPoint().distanceTo(Players.local.getWorldLocation()) > 20) {
                        teleports.add(
                            Teleport(tab.getPoint(), 5
                            ) {
        ...
        if (getWildyLevel() <= 20) {
            for (teleportSpell in TeleportSpell.values()) {
                ...
                if (teleportSpell.getPoint().distanceTo(Players.local.getWorldLocation()) > 50) {
                    teleports.add(
                        Teleport(teleportSpell.getPoint(), 5
    ...
    fun jewelryPopupTeleport(target: String?, vararg ids: Int) {
        val inv: Item? = Inventory.getFirst(ids)
        if (inv != null) {
            val baseWidget: Widget? = Widgets.get(187, 3)
