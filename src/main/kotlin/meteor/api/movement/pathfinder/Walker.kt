...
/*        if (path.isEmpty()) {
    ...
    private fun reachablePath(remainingPath: List<WorldPoint?>): List<WorldPoint> {
        val local: Player = Players.local
    ...
    fun step(destination: WorldPoint): Boolean {
        ...
        if (!Movement.isRunEnabled && (client.energy >= Rand.nextInt(
                MIN_ENERGY,
                MAX_MIN_ENERGY
            ) || local.healthScale > -1 && client.energy > 0)
        ) {
            Movement.toggleRun()
            Time.sleepUntil(Movement::isRunEnabled, 2000)
            return true
        }
        if (!Movement.isRunEnabled && client.energy > 0 && Movement.isStaminaBoosted) {
            Movement.toggleRun()
            Time.sleepUntil(Movement::isRunEnabled, 2000)
            return true
        }
        ...
        for (i in 0 until MAX_INTERACT_DISTANCE) {
            ...
            if (transportTargets != null) {
                ...
                if (transport != null && local.worldLocation.distanceTo(transport.source) <= transport.sourceRadius) {
                    logger.debug("Trying to use transport at {}", transport.source.toString())
                    transport.handler?.run()
                    Time.sleep(2800)
                    return true
                }
            }
            val tileB: Tile = Tiles.getAt(b!!) ?: return false
    ...
    fun remainingPath(path: List<WorldPoint?>?): List<WorldPoint?> {
        val local: Player = client.localPlayer ?: return emptyList<WorldPoint>()
    ...
    private val globalCollisionMap: GlobalCollisionMap? = null
    private fun getGlobalCollisionMap(): GlobalCollisionMap? {
        return globalCollisionMap
    }
    private fun calculatePath(
        startPoints: MutableList<WorldPoint?>,
        destination: WorldPoint?,
        transports: Map<WorldPoint, MutableList<Transport>>,
        local: Boolean
    ): List<WorldPoint> {
        val collisionMap: CollisionMap = (if (local) LocalCollisionMap() else getGlobalCollisionMap())
            ?: return emptyList()
    ...
    @JvmStatic
    fun buildTransportLinks(): Map<WorldPoint, MutableList<Transport>> {
        val out: MutableMap<WorldPoint, MutableList<Transport>> = HashMap()
    ...
    fun buildPath(destination: WorldPoint?, localRegion: Boolean): List<WorldPoint> {
        ...
        val transports: Map<WorldPoint, MutableList<Transport>> = buildTransportLinks()
        val teleports: LinkedHashMap<WorldPoint?, Teleport> = buildTeleportLinks(destination)
        val startPoints: MutableList<WorldPoint?> = ArrayList(teleports.keys)
