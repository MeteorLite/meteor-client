...
object Worlds {
    ...
    private fun lookup(): List<World> {
        ...
        lookup.getWorlds().forEach(Consumer { w: net.runelite.http.api.worlds.World ->
    ...
    fun getAll(filter: Predicate<World>): List<World?> {
        ...
        loadedWorlds = try {
            val worlds: Array<World> = client.getWorldList()
            if (worlds == null) {
                loadWorlds()
                return out
            }
    ...
    fun hopTo(world: World?, spam: Boolean = false) {
        ...
        if (!GameThread.invokeLater { rememberOption.isHidden }!!) {
            chooseOption(2)
            Time.sleepUntil({ client.getGameState() ===GameState.HOPPING }, 3000)
            return
        }
    ...
    val currentWorld: World?
        get() = getFirst(client.getWorld())

    fun inMembersWorld(): Boolean {
        return lookup().stream().filter { x: World -> x.id == currentId }
            .findFirst()
            .get().isMembers
    }
