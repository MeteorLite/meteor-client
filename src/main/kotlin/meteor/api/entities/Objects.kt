...
object Objects : Entity<TileObject>() {
    override fun all(filter: Predicate<in TileObject>): List<TileObject> {
        return getTileObjects(Tiles.tiles).stream()
            ...
            .collect(Collectors.toList()) as List<TileObject>
    }

    private val TILE_OBJECTS = Objects
    ...
    fun getAt(tile: Tile?, vararg ids: Int): List<TileObject?> {
    ...
    fun getAt(tile: Tile?, vararg names: String): List<TileObject> {
        return getAt(tile) ret@{ x: TileObject ->
            if (x.name == null) {
                return@ret false
            }
            for (name in names) {
    ...
    private fun parseTile(tile: Tile?, pred: Predicate<TileObject>): List<TileObject> {
        val filter = Predicate { x: TileObject? ->
    ...
    private fun getTileObjects(tile: Tile): List<TileObject> {
        val out: ArrayList<TileObject> = ArrayList()
        ...
        if (gameObjects != null) {
            for (gameObject in gameObjects) {
                if (gameObject == null || !client.isTileObjectValid(tile, gameObject)
                    || gameObject.id == -1
                ) {
                    continue
                }
